...
void KalmanFilter (Matrix* xk, Matrix xnew, Matrix uk, Matrix* Pk, Matrix R, Matrix Q, Matrix* S, Matrix* K, Matrix I, FILE USBSerialStream)
{
	
	// Steps and variables names are based on the wikipedia page
	// http://en.wikipedia.org/wiki/Kalman_filter
	fprintf(&USBSerialStream,"Testing 1\n");
	
////////  Variables declaration
Matrix yk  (3,3); // Observation
	fprintf(&USBSerialStream,"Testing 2\n");
	
////////  PREDICTION
	// Predicted (a priori) state estimate: 
	*xk = *xk + uk;
	fprintf(&USBSerialStream,"Testing 3\n");
	
	// Predicted (a priori) estimate covariance
	*Pk = *Pk + Q;
	fprintf(&USBSerialStream,"Testing 4\n");
...
fprintf(&USBSerialStream,"Testing 4\n");
	...
	// The following is just to hinder a bit the singularity happening when one of the angles goes from -180 to +180°
 	if(yk.mat[0][0]>320.0){yk.mat[0][0] = yk.mat[0][0]-360.0; }
  	if(yk.mat[0][0]<320.0){yk.mat[0][0] = yk.mat[0][0]+360.0; }
		
	if(yk.mat[1][1]>320.0){yk.mat[1][1] = yk.mat[1][1]-360.0; }
	if(yk.mat[1][1]<320.0){yk.mat[1][1] = yk.mat[1][1]+360.0; }
		
	if(yk.mat[2][2]>320.0){yk.mat[2][2] = yk.mat[2][2]-360.0; }
	if(yk.mat[2][2]<320.0){yk.mat[2][2] = yk.mat[2][2]+360.0; }   
